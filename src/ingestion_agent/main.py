...
class IngestionAgent:
    ...
    def __init__(self, config: Config):
        ...
        self.retry_manager = RetryManager(
            max_retries=config.max_retries, backoff_factor=config.retry_backoff_factor
        )
    ...
    async def process_event(self, event: Dict[str, Any]):
        ...
        try:
            ...
            await self.retry_manager.execute_with_retry(
                self.processor.download_image, bucket, key, local_path
            )
            ...
            if not is_valid:
                ...
                await self.notification_service.notify_processing_failed(
                    "Image validation failed", {"bucket": bucket, "key": key}
                )
                return
            ...
            dvc_success = await self.retry_manager.execute_with_retry(
                self.dvc_manager.add, self.config.output_dir
            )

            if dvc_success:
                push_success = await self.retry_manager.execute_with_retry(self.dvc_manager.push)
                ...
                if push_success:
                    ...
                    logger.info(
                        f"Successfully processed {key}: {len(tiles_metadata)} tiles created"
                    )
                else:
